...
async function loadSteamRecommendations() {
  ...
  const sorted = Object.values(map)
    .filter(app => !LEGACY_RECOMMENDATION_EXCLUDES.has(Number(app.appid)))
    .filter(app => !LEGACY_RECOMMENDATION_TITLES.test(String(app.name||'')))
    .filter(app => POPULAR_GAME_IDS.has(Number(app.appid)) || (app.ccu||0) >= 3000 || (app.owners||0) >= 2000000 || (app.positive||0) >= 15000)
    .map(app => {
      const recentBoost = POPULAR_GAME_IDS.has(Number(app.appid)) ? 20 : 0;
      const qualityBoost = Math.log1p(app.ccu||0) * 1.9 + Math.log1p(app.positive||0) * 0.35 + Math.log1p(app.owners||0) * 0.55;
      return { ...app, _score: app._score + recentBoost + qualityBoost };
    })
    .sort((a,b)=>b._score-a._score)
    .slice(0,120);
  if (!sorted.length) {
