...
const EXTRA_GAMES = [
  ...
  { id:3419430, title:"Bongo Cat",                    genre:"인디",     tags:["캐주얼","인디","고양이","밈","무료"],             rating:8.8, year:2025, dev:"Marcel Zurawka, Julius Krüger", desc:"키보드를 두드릴 때마다 책상 위 고양이가 반응하는 캐주얼 밈 게임." },
  { id:2767030, title:"Marvel Rivals",                genre:"FPS",      tags:["히어로","팀","슈터","마블","멀티플레이어"],      rating:8.4, year:2024, dev:"NetEase Games", desc:"마블 영웅 팀을 조합해 싸우는 팀 기반 PvP 슈터." },
  { id:1422450, title:"Deadlock",                     genre:"FPS",      tags:["슈터","MOBA","팀","전략","멀티플레이어"],        rating:8.3, year:2025, dev:"Valve", desc:"슈터와 MOBA 요소를 결합한 밸브의 팀 기반 멀티플레이어 게임." },
  { id:3241660, title:"R.E.P.O.",                     genre:"공포",     tags:["공포","협동","추출","물리","온라인"],            rating:8.9, year:2025, dev:"semiwork", desc:"물리 기반 오브젝트를 회수하며 탈출하는 온라인 협동 호러 게임." },
  { id:3405690, title:"EA SPORTS FC 26",             genre:"스포츠",   tags:["축구","스포츠","멀티플레이어","경쟁","시뮬"],    rating:7.8, year:2025, dev:"EA Canada, EA Romania", desc:"EA SPORTS FC 시리즈 최신 축구 게임." },
  { id:2943650, title:"FragPunk",                    genre:"FPS",      tags:["히어로","슈터","5대5","카드","경쟁"],            rating:8.1, year:2025, dev:"Bad Guitar Studio", desc:"샤드 카드로 전투 규칙을 뒤트는 5대5 히어로 슈터." },
  { id:3167020, title:"Escape from Duckov",          genre:"서바이벌", tags:["추출","PvE","오픈월드","은신처","오리"],         rating:8.5, year:2025, dev:"Team Soda", desc:"오리 세계관에서 파밍과 은신처 건설을 하는 PvE 탈출 서바이벌." },
...
// CONSTANTS & CONFIG
// ══════════════════════════════════════════════════════════════
const STORAGE_KEY = 'nexus-play-v3';
...
const GENRE_STEAM_TAGS = {
  ...
  '스포츠':  ['Sports','Soccer','Basketball'],
  '샌드박스':['Open World','Sandbox','Building'],
...
async function loadLocalData(path) {
  ...
  if (_sessionDataCache.has(path)) return _sessionDataCache.get(path);
  try {
    const res = await fetch(DATA_BASE + path, { cache: 'force-cache' });
    if (!res.ok) return null;
    const d = await res.json();
    if (!d || d._note || Object.keys(d).length < 5) return null;
    _sessionDataCache.set(path, d);
    return d;
  } catch { return null; }
}
...
function steamThumb(id)    { return `https://cdn.akamai.steamstatic.com/steam/apps/${id}/header.jpg`; }
function steamThumbAlt(id) { return `https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/${id}/header.jpg`; }
function steamCardThumb(app) {
  const override = getThumbOverride(app?.name ? { title: app.name } : null, app?.name || "");
  return override?.image || steamThumb(app.appid);
}
function steamCardThumbAlt(app) {
  const override = getThumbOverride(app?.name ? { title: app.name } : null, app?.name || "");
  return override?.image || steamThumbAlt(app.appid);
}
function steamCardLink(app) {
  return storeUrl(app?.appid || 0, app?.name || "");
}
  ...
  return `
    <a class="steam-card" href="${steamCardLink(app)}" target="_blank" rel="noopener">
      <div class="steam-card-img-wrap">
        <img class="steam-card-img" src="${steamCardThumb(app)}" alt="${(app.name||'').replace(/"/g,'')}" loading="lazy"
             data-appid="${app.appid}" onerror="handleSteamImgError(this, ${app.appid}, ['${steamCardThumbAlt(app)}'])">
...
async function loadSteamRecommendations() {
  ...
  heroEl.innerHTML = sorted.slice(0,3).map((app,i)=>`
    <div class="hero-card" onclick="window.open(steamCardLink(app),'_blank')" style="animation-delay:${i*0.08}s">
      <img src="${steamCardThumb(app)}" alt="${app.name}" onerror="this.src='${steamThumbAlt(app.appid)}';this.onerror=null">
      <div class="hero-card-overlay">
        <div class="hero-rank">${i+1}</div>
  ...
  moreEl.innerHTML = sorted.slice(3,120).map(app=>makeSteamCard(app,null)).join('');
}
...
function applyGenreGrid(list, genre) {
  ...
  const filtered = q
    ? list.filter(a => (a.name||'').toLowerCase().includes(q) || Object.keys(a.tags||{}).some(t => t.toLowerCase().includes(q)))
    : list;
  gridEl.innerHTML = filtered.length
    ? filtered.map(app=>makeSteamCard(app,null,{ showLive:false })).join('')
    : `<div class="grid-empty" style="grid-column:1/-1">${genre} 검색 결과 없음</div>`;
}
...
async function loadSteamGenreData(genre, opts = {}) {
  ...
  if (_steamGenreCache[genre]?.length) {
  ...
  results.forEach(d=>{
    ...
    Object.values(d).forEach(app=>{
      if(!app.appid||!app.name) return;
      if(!merged[app.appid]) merged[app.appid]={...app,_cnt:0,_source:'steam'};
      merged[app.appid]._cnt++;
    });
  });
  ...
  let sorted = Object.values(merged)
    ...
    .slice(0,240)
    .map(app => normalizeGenreItem(app, genre))
    .filter(Boolean);

  const localFallback = GAMES_CLEAN
    .filter(g => genre === '전체' || g.genre === genre)
    .map(g => normalizeGenreItem(g, genre))
    .filter(Boolean);

  sorted = uniqueByAppId([...sorted, ...localFallback]);
  if (genre === '전체') {
    sorted = [...sorted].sort((a,b)=>((b.ccu||0)-(a.ccu||0))||((b.positive||0)-(a.positive||0))||((b.average_2weeks||0)-(a.average_2weeks||0))||((b.rating||0)-(a.rating||0))||String(a.name||'').localeCompare(String(b.name||'')));
  }
...
function setHotSort(by) {
  ...
  document.querySelectorAll('.hot-sort-btn').forEach((b,i)=>
    b.classList.toggle('active',(i===0&&by==='ccu')||(i===1&&by==='positive')||(i===2&&by==='average_2weeks')));
  renderHotGrid();
}
...
function buildHotFallback() {
  const base = GAMES_CLEAN.map((g,i) => normalizeGenreItem({
    ...
    average_2weeks: Math.round((11 - (g.rating||7)) * 35 + ((g.year||2020) >= 2022 ? 260 : 40) + (i % 17) * 4),
    genre: g.genre
  }, g.genre));
  return {
    ccu: [...base].sort((a,b)=>(b.ccu||0)-(a.ccu||0)),
    positive: [...base].sort((a,b)=>((b.positive||0)-(a.positive||0))||((a.negative||0)-(b.negative||0))),
    average_2weeks: [...base].sort((a,b)=>(b.average_2weeks||0)-(a.average_2weeks||0)),
  };
}
