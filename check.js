...
function matchesGenreApp(app, genre) {
  ...
  const overrideGenre = EXACT_GENRE_OVERRIDES[appid];
  if (overrideGenre) return overrideGenre === genre;
  const local = getGameById(appid);
  if (local?.genre) return local.genre === genre;
  if ((app._genre || app.genre) === genre) return true;
